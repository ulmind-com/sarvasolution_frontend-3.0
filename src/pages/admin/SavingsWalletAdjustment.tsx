...
const SavingsWalletAdjustment = () => {
  ...
  const fetchLogs = async () => {
    try {
      setLogsLoading(true);
      const res = await api.get('/api/v1/admin/savings-wallet/adjustment-logs');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const res = await api.post('/api/v1/admin/savings-wallet/adjust', {
        ...formData,
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <Button type="submit" disabled={isLoading || !!memberError || !memberName} className="w-full">
                {isLoading ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : <PiggyBank className="w-4 h-4 mr-2" />}
...
export default SavingsWalletAdjustment;
