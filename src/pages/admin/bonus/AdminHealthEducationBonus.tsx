...
import {
    getHealthEducationBonusLivePool,
    getHealthEducationBonusPools,
    triggerHealthEducationBonus,
    applyHealthEducationBonusCredits
...
export default function AdminHealthEducationBonus() {
    ...
    const fetchUsers = async () => {
        ...
        try {
            const res = await getHealthEducationBonusLivePool();
    ...
    const fetchPools = async (page: number) => {
        ...
        try {
            const res = await getHealthEducationBonusPools(page, 12);
    ...
    const handleTrigger = async () => {
        ...
        try {
            ...
            const res = await triggerHealthEducationBonus(y, m);
            if (res) {
                toast({ title: "Trigger Successful", description: "Health & Education Bonus month-end computation staged." });
                fetchPools(1);
            }
        } catch (error: any) {
    ...
    const handleApplyCredits = async () => {
        ...
        try {
            const y = parseInt(actionYear);
            const m = parseInt(actionMonth);
            const res = await applyHealthEducationBonusCredits(y, m);
            if (res) {
                toast({ title: "Credits Applied", description: "Health & Education Bonus wallet credits applied successfully." });
                fetchPools(1);
            }
        } catch (error: any) {
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-card p-6 rounded-lg glass premium-shadow border-primary/10">
                <div>
                    <h1 className="text-2xl md:text-3xl font-bold tracking-tight">Health & Education Bonus Admin</h1>
                    <p className="text-muted-foreground mt-1">
                        Monitor live eligible users, manage monthly pools — <strong>50,000 BV = 1 Unit, No Capping, 5% Pool</strong>
                    </p>
                </div>
                ...
                <div className="flex items-center gap-2 p-3 bg-muted/30 rounded-lg border border-border/50">
                    ...
                    <AlertDialog>
                        ...
                        <AlertDialogContent className="glass">
                            <AlertDialogHeader>
                                ...
                                <AlertDialogDescription>
                                    This will compute Health & Education Bonus pool amounts and unit values for <strong>{actionMonth}/{actionYear}</strong>.
                    ...
                    <AlertDialog>
                        ...
                        <AlertDialogContent className="glass border-green-500/20">
                            <AlertDialogHeader>
                                <AlertDialogTitle className="text-green-600">Apply Health & Education Bonus Credits?</AlertDialogTitle>
                                <AlertDialogDescription>
            ...
            <Tabs defaultValue="estimates" className="w-full">
                ...
                <TabsContent value="estimates">
                    <div className="space-y-6">
                        ...
                        {livePoolData && (
                            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                                ...
                                <Card className="glass premium-shadow border-rose-500/20 bg-rose-500/5">
                                    <CardHeader className="pb-2">
                                        <CardTitle className="text-sm font-medium text-rose-600">5% Pool Amount</CardTitle>
                                    </CardHeader>
                                    <CardContent>
                                        <div className="text-2xl font-bold text-rose-600">₹{livePoolData.poolAmount?.toLocaleString('en-IN', { maximumFractionDigits: 2 }) || 0}</div>
                                    </CardContent>
                                </Card>
                                <Card className="glass premium-shadow">
                                    <CardHeader className="pb-2">
                                    ...
                                    <CardContent>
                                        ...
                                        <p className="text-xs text-muted-foreground mt-1">{livePoolData.eligibleUserCount || 0} Eligible · 50,000 BV/Unit</p>
                                    </CardContent>
                                </Card>
                        ...
                        <Card className="glass premium-shadow overflow-hidden border-t-4 border-t-rose-500">
                            <CardHeader className="flex flex-row items-center justify-between bg-muted/20 pb-4">
                                <div>
                                    ...
                                    <CardDescription>Real-time Health & Education Bonus unit estimates — no capping applied.</CardDescription>
                                </div>
                                            ...
                                            {loadingUsers ? (
                                                ...
                                                <TableRow>
                                                    <TableCell colSpan={7} className="h-40 text-center text-muted-foreground">
                                                        No active members currently qualifying for the Health & Education Bonus pool.
                                                    </TableCell>
                                                </TableRow>
                                            )}
                                        </TableBody>
                    ...
                    <Card className="glass premium-shadow overflow-hidden">
                        <CardHeader className="flex flex-row items-center justify-between bg-muted/20 pb-4">
                            <div>
                                ...
                                <CardDescription>History of Health & Education Bonus pool computations and payouts.</CardDescription>
                            </div>
                                ...
                                <Table>
                                    <TableHeader className="bg-muted/50">
                                        <TableRow>
                                            ...
                                            <TableHead className="text-rose-600">5% Pool Cash</TableHead>
                                        ...
                                        {loadingPools ? (
                                            ...
                                            pools.map((p, i) => (
                                                <TableRow key={i} className="hover:bg-accent/40">
                                                    ...
                                                    <TableCell className="font-bold text-rose-600">₹{(p.poolAmount || 0).toLocaleString('en-IN')}</TableCell>
                                            ...
                                            <TableRow>
                                                <TableCell colSpan={7} className="h-40 text-center text-muted-foreground">
                                                    No health & education bonus pools have been staged yet.
                                                </TableCell>
                                            </TableRow>
                                        )}
                                    </TableBody>
