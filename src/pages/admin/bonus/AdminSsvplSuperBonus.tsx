...
import {
    getSsvplSuperBonusLivePool,
    getSsvplSuperBonusPools,
    getActiveSsvplSuperBonusUsers,
    triggerSsvplSuperBonus,
    applySsvplSuperBonusCredits
...
export default function AdminSsvplSuperBonus() {
    ...
    // Default to the current financial year cycle
    ...
    const fetchUsers = async () => {
        ...
        try {
            ...
            try {
                const res = await getSsvplSuperBonusLivePool();
            ...
            const res = await getActiveSsvplSuperBonusUsers();
            ...
            console.error("Failed to fetch live SSVPL Super Bonus data:", error);
        } finally {
            setLoadingUsers(false);
        }
    };
    ...
    const fetchPools = async (page: number) => {
        ...
        try {
            const res = await getSsvplSuperBonusPools(page, 12);
            ...
            console.error("Failed to fetch super bonus pools:", error);
        } finally {
    ...
    const handleTrigger = async () => {
        ...
        try {
            ...
            const res = await triggerSsvplSuperBonus(y);
            if (res) {
                toast({ title: "Trigger Successful", description: "SSVPL Super Bonus annual computation staged." });
                fetchPools(1);
            }
        } catch (error: any) {
    ...
    const handleApplyCredits = async () => {
        ...
        try {
            const y = parseInt(actionYear);
            const res = await applySsvplSuperBonusCredits(y);
            if (res) {
                toast({ title: "Credits Applied", description: "SSVPL Super Bonus wallet credits applied successfully." });
                fetchPools(1);
            }
        } catch (error: any) {
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-gradient-to-br from-purple-500/10 to-transparent border border-purple-500/20 p-6 rounded-lg glass premium-shadow">
                <div className="flex items-start gap-4">
                    <div className="bg-purple-500/20 p-3 rounded-xl hidden md:block">
                        <BadgeCheck className="h-8 w-8 text-purple-600" />
                    </div>
                    <div>
                        <h1 className="text-2xl md:text-3xl font-bold tracking-tight text-purple-950 dark:text-purple-50">SSVPL Super Bonus (Yearly)</h1>
                        <p className="text-purple-700 dark:text-purple-400 mt-1">
                            Monitor live eligible users, manage 12-month pools — <strong>25,00,000 BV = 1 Unit, No Capping, 2% Pool</strong>
                        </p>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-2 p-3 bg-card rounded-lg border border-border/50 shadow-sm">
                    ...
                    <AlertDialog>
                        <AlertDialogTrigger asChild>
                            <Button size="sm" variant="outline" disabled={triggering} className="gap-1 border-purple-500/30 text-purple-600 hover:bg-purple-500/10">
                                <Zap className="h-4 w-4" /> {triggering ? "..." : "Stage"}
                            </Button>
                        </AlertDialogTrigger>
                        <AlertDialogContent className="glass border-purple-500/30">
                            <AlertDialogHeader>
                                <AlertDialogTitle>Stage Annual Pool Distribution?</AlertDialogTitle>
                                <AlertDialogDescription>
                                    This will compute SSVPL Super Bonus pool amounts and unit values for <strong>Year {actionYear}</strong>.
                                    It creates wallet credit records but does <strong>NOT</strong> apply the money to wallets yet.
                                </AlertDialogDescription>
                            </AlertDialogHeader>
                            <AlertDialogFooter>
                                <AlertDialogCancel>Cancel</AlertDialogCancel>
                                <AlertDialogAction onClick={handleTrigger} className="bg-purple-600 text-white hover:bg-purple-700">Confirm Stage</AlertDialogAction>
                            </AlertDialogFooter>
                        </AlertDialogContent>
                    </AlertDialog>
                    ...
                    <AlertDialog>
                        <AlertDialogTrigger asChild>
                            <Button size="sm" disabled={applying} className="gap-1 bg-purple-500 hover:bg-purple-600 text-white shadow-lg shadow-purple-500/20">
                                <CheckCircle2 className="h-4 w-4" /> {applying ? "..." : "Apply"}
                            </Button>
                        </AlertDialogTrigger>
                        <AlertDialogContent className="glass border-green-500/30">
                            <AlertDialogHeader>
                                <AlertDialogTitle className="text-green-600">Apply Super Bonus Credits?</AlertDialogTitle>
                                <AlertDialogDescription>
            ...
            <Tabs defaultValue="estimates" className="w-full">
                    ...
                    <div className="space-y-6">
                        ...
                        {livePoolData && livePoolData.companyTotalBV !== undefined && (
                            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                                <Card className="glass premium-shadow border-purple-500/10">
                                ...
                                <Card className="glass premium-shadow border-purple-500/30 bg-gradient-to-b from-purple-500/5 to-purple-500/10">
                                    <CardHeader className="pb-2">
                                        <CardTitle className="text-sm font-medium text-purple-700 border-purple-500">2% Annual Pool Cash</CardTitle>
                                    </CardHeader>
                                    <CardContent>
                                        <div className="text-2xl font-bold text-purple-700">₹{livePoolData.poolAmount?.toLocaleString('en-IN', { maximumFractionDigits: 2 }) || 0}</div>
                                    </CardContent>
                                </Card>
                                <Card className="glass premium-shadow border-purple-500/10">
                                    <CardHeader className="pb-2">
                                    ...
                                    <CardContent>
                                        ...
                                        <p className="text-xs text-muted-foreground mt-1">{livePoolData.eligibleUserCount || 0} Eligible · 25L BV/Unit</p>
                                    </CardContent>
                                </Card>
                        ...
                        <Card className="glass premium-shadow overflow-hidden border-t-4 border-t-purple-500">
                            <CardHeader className="flex flex-row items-center justify-between bg-purple-50/50 dark:bg-purple-950/20 pb-4">
                                <div>
                                    <CardTitle>Active 12-Month Cycle Tracker</CardTitle>
                                    <CardDescription>Real-time SSVPL Super Bonus unit estimates for top performers.</CardDescription>
                                </div>
                                <Button variant="ghost" size="icon" onClick={fetchUsers}>
                                    <RefreshCw className={`h-4 w-4 ${loadingUsers ? 'animate-spin text-purple-600' : 'text-purple-600'}`} />
                                </Button>
                            </CardHeader>
                                    ...
                                    <Table>
                                        ...
                                        <TableBody>
                                            {loadingUsers ? (
                                                ...
                                                activeUsers.map((u, i) => (
                                                    <TableRow key={i} className="hover:bg-purple-50/50 dark:hover:bg-purple-900/20">
                                                        <TableCell>
                                                            <div className="font-medium text-purple-950 dark:text-purple-100 flex items-center gap-2">
                                                                {u.fullName || u.memberId}
                                                                {(u.finalUnits || u.projectedUnits || 0) >= 2 && <Crown className="h-3.5 w-3.5 text-purple-500" />}
                                                            </div>
                                                        ...
                                                        <TableCell className="font-semibold text-purple-700/80 dark:text-purple-400">
                                                            {(u.adjustedWeakerLeg || Math.min(u.adjustedLeft || u.leftBV || 0, u.adjustedRight || u.rightBV || 0)).toLocaleString()} BV
                                                        </TableCell>
                                                        <TableCell>
                                                            <Badge variant="default" className="bg-purple-500 hover:bg-purple-600 shadow-sm text-white">
                                                ...
                                                <TableRow>
                                                    <TableCell colSpan={7} className="h-40 text-center text-muted-foreground">
                                                        No members currently qualifying for the annual SSVPL Super Bonus pool.
                                                    </TableCell>
                                                </TableRow>
                                            )}
                                        </TableBody>
                ...
                <TabsContent value="pools">
                    <Card className="glass premium-shadow overflow-hidden border-purple-500/10">
                        <CardHeader className="flex flex-row items-center justify-between bg-purple-50/50 dark:bg-purple-950/20 pb-4">
                            <div>
                                <CardTitle>Pool Distributions</CardTitle>
                                <CardDescription>History of annual SSVPL Super Bonus pool computations and payouts.</CardDescription>
                            </div>
                            <Button variant="ghost" size="icon" onClick={() => fetchPools(1)}>
                                <RefreshCw className={`h-4 w-4 ${loadingPools ? 'animate-spin text-purple-600' : 'text-purple-600'}`} />
                            </Button>
                        </CardHeader>
                        <CardContent className="p-0">
                            <div className="overflow-x-auto">
                                <Table>
                                    <TableHeader className="bg-muted/30">
                                        <TableRow>
                                            ...
                                            <TableHead className="text-purple-700">2% Pool Cash</TableHead>
                                    ...
                                    <TableBody>
                                        {loadingPools ? (
                                            ...
                                            pools.map((p, i) => (
                                                <TableRow key={i} className="hover:bg-purple-50/50 dark:hover:bg-purple-900/20">
                                                    <TableCell className="font-bold text-purple-900 dark:text-purple-100 whitespace-nowrap flex items-center gap-2">
                                                        <BadgeCheck className="h-4 w-4 text-purple-500" />
                                                        Year {p.cycleYear || p.year}
                                                    </TableCell>
                                                    <TableCell>{(p.companyTotalBV || 0).toLocaleString()}</TableCell>
                                                    <TableCell className="font-bold text-purple-700">₹{(p.poolAmount || 0).toLocaleString('en-IN')}</TableCell>
                                            ...
                                            <TableRow>
                                                <TableCell colSpan={7} className="h-40 text-center text-muted-foreground">
                                                    No annual SSVPL super bonus pools have been staged yet.
                                                </TableCell>
                                            </TableRow>
                                        )}
                                    </TableBody>
                            ...
                            {poolPagination.totalPages > 1 && (
                                <div className="flex items-center justify-between px-4 py-3 border-t bg-purple-50/30 dark:bg-purple-950/10">
