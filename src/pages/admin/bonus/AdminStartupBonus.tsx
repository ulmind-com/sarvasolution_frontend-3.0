...
import {
    getStartupBonusLivePool,
    getStartupBonusPools,
    triggerStartupBonus,
    applyStartupBonusCredits
...
export default function AdminStartupBonus() {
    ...
    const fetchUsers = async () => {
        ...
        try {
            const res = await getStartupBonusLivePool();
    ...
    const fetchPools = async (page: number) => {
        ...
        try {
            const res = await getStartupBonusPools(page, 12);
    ...
    const handleTrigger = async () => {
        ...
        try {
            ...
            const res = await triggerStartupBonus(y, m);
            if (res) {
                toast({
                    title: "Trigger Successful",
                    description: "Startup Bonus month-end computation staged.",
                });
                fetchPools(1);
            }
        } catch (error: any) {
    ...
    const handleApplyCredits = async () => {
        ...
        try {
            const y = parseInt(actionYear);
            const m = parseInt(actionMonth);
            const res = await applyStartupBonusCredits(y, m);
            if (res) {
                toast({
                    title: "Credits Applied",
                    description: "Startup Bonus wallet credits applied successfully.",
                });
                fetchPools(1);
            }
        } catch (error: any) {
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-card p-6 rounded-lg glass premium-shadow border-primary/10">
                <div>
                    <h1 className="text-2xl md:text-3xl font-bold tracking-tight">Start Up Bonus Admin</h1>
                    <p className="text-muted-foreground mt-1">
                        Monitor live eligible users, manage monthly pools — <strong>5000 BV = 1 Unit, No Capping</strong>
                    </p>
                </div>
                ...
                <div className="flex items-center gap-2 p-3 bg-muted/30 rounded-lg border border-border/50">
                    ...
                    <AlertDialog>
                        ...
                        <AlertDialogContent className="glass">
                            <AlertDialogHeader>
                                ...
                                <AlertDialogDescription>
                                    This will compute Startup Bonus pool amounts and unit values for <strong>{actionMonth}/{actionYear}</strong>.
                    ...
                    <AlertDialog>
                        ...
                        <AlertDialogContent className="glass border-green-500/20">
                            <AlertDialogHeader>
                                <AlertDialogTitle className="text-green-600">Apply Startup Bonus Credits?</AlertDialogTitle>
                                <AlertDialogDescription>
                                    This will irrevocably apply the staged startup bonus to user wallets for <strong>{actionMonth}/{actionYear}</strong>.
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                                ...
                                <Card className="glass premium-shadow">
                                    ...
                                    <CardContent>
                                        ...
                                        <p className="text-xs text-muted-foreground mt-1">{livePoolData.eligibleUserCount || 0} Eligible · 5000 BV/Unit</p>
                                    </CardContent>
                                </Card>
                        ...
                        <Card className="glass premium-shadow overflow-hidden border-t-4 border-t-primary">
                            <CardHeader className="flex flex-row items-center justify-between bg-muted/20 pb-4">
                                <div>
                                    ...
                                    <CardDescription>Real-time Startup Bonus unit estimates — no capping applied.</CardDescription>
                                </div>
                                            ...
                                            {loadingUsers ? (
                                                ...
                                                <TableRow>
                                                    <TableCell colSpan={7} className="h-40 text-center text-muted-foreground">
                                                        No active members currently qualifying for the Startup Bonus pool.
                                                    </TableCell>
                                                </TableRow>
                                            )}
                                        </TableBody>
                    ...
                    <Card className="glass premium-shadow overflow-hidden">
                        <CardHeader className="flex flex-row items-center justify-between bg-muted/20 pb-4">
                            <div>
                                ...
                                <CardDescription>History of Startup Bonus pool computations and payouts.</CardDescription>
                            </div>
                                ...
                                <Table>
                                    ...
                                    <TableBody>
                                        {loadingPools ? (
                                            ...
                                            <TableRow>
                                                <TableCell colSpan={7} className="h-40 text-center text-muted-foreground">
                                                    No startup bonus pools have been staged yet.
                                                </TableCell>
                                            </TableRow>
                                        )}
                                    </TableBody>
